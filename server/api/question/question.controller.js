...
module.exports = {
  createQuestion: (req, res) => {
    ...
    if (!question || !question_description)
      return res.status(400).json({ msg: "Please provide a title for your question." });

    createQuestion(req.body, (err, results) => {
      if (err) {
        console.log(err);
        return res.status(500).json({ msg: "Error adding the question: database connection err" });
      }
